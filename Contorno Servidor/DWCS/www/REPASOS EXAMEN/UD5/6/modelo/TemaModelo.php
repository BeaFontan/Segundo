...
class TemaModelo extends Tema
{
    ...
    //Buscar por titulo
    public function buscar(): array|string
    {
        $pdo = new Conexion();
        ...
        try {
            ...
            return "Erro bucando por título " . $e->getMessage();
        }
    }
    ...
    //Editar
    public function editar(string $tituloAEditar): string
    {
        $pdo = new Conexion();

        try {
            $query = $pdo->prepare("UPDATE `tema` SET `Titulo`=?,`Autor`=?,`Ano`=?,`Duracion`=?,`Imaxe`=? WHERE Titulo like ?");
            $query->execute([$this->titulo, $this->autor, $this->ano, $this->duracion, $this->imaxe, $tituloAEditar]);
    ...
    public static function mostrarTodos(): array|string
    {
        ...
        try {
            ...
            return "Erro devolvendo todos " . $e->getMessage();
        }
    }
}